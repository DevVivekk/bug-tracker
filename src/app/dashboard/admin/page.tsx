...
const Page = () => {
  ...
  const submitfn = async()=>{
     try {
        ...
        const req = {
          method: "GET",
          data: {},
          path: `${process.env.NEXT_PUBLIC_URL}/server/api/v1/admin/get-all-bugs`
        }
        const ans = await callApi(req)
        if (!ans.status) {
          setError(true)
        } else {
          setData(ans.data as BugData[])
        }
      } catch (err) {
        console.error(err)
        setError(true)
  ...
  const filteredBugs = data.flatMap(user => 
    user.bugs
      .filter(bug => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-6 flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 text-black pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
              />
            </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800">
                  {data.reduce((total, user) => total + user.bugs.length, 0)}
                </h3>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800">
                  {data.reduce((total, user) => total + user.bugs.filter(bug => bug.status === 'open').length, 0)}
                </h3>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800">
                  {data.reduce((total, user) => total + user.bugs.filter(bug => bug.status === 'onhold').length, 0)}
                </h3>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800">
                  {data.reduce((total, user) => total + user.bugs.filter(bug => bug.status === 'closed').length, 0)}
                </h3>
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 flex flex-row justify-start items-start flex-wrap">
            {filteredBugs.map(({ bug, user }, index) => (
              <BugComp
                key={bug._id+index}
                isEditing={editMode}
                isAdmin={true}
                ...
                fn={submitfn}
                data={bug}
                reporter={{ username: user.username, email: user.email }}
              />
            ))}
          </div>
        ) : (
